...
void pwm_init(){
	...
	export_fd = open("/sys/class/pwm/pwmchip2/export", O_WRONLY);
    write(export_fd, "0", 1);
    close(export_fd);

	export_fd = open("/sys/class/pwm/pwmchip4/export", O_WRONLY);
    write(export_fd, "0", 1);
    close(export_fd);
...
void pwm_configure(){
	int export_fd, period_fd, duty_cycle_fd, enable_fd;
	period_fd = open("/sys/class/pwm/pwmchip2/pwm0/period", O_WRONLY);
    ...
    period_fd = open("/sys/class/pwm/pwmchip4/pwm0/period", O_WRONLY);
    write(period_fd, "1000000000", 10);
    close(period_fd);

}
void led_blue_on(){ //p9_22
	int export_fd, period_fd, duty_cycle_fd, enable_fd;
...
void led_red_on(){ // p9_14
	int export_fd, period_fd, duty_cycle_fd, enable_fd;
	duty_cycle_fd = open("/sys/class/pwm/pwmchip2/pwm0/duty_cycle", O_WRONLY);
    write(duty_cycle_fd, "500000000", 9);
    close(duty_cycle_fd);
	sleep(1);
    enable_fd = open("/sys/class/pwm/pwmchip2/pwm0/enable", O_WRONLY);
    write(enable_fd, "1", 1);
    close(enable_fd);
}

void led_green_on(){ //p8_19
	int export_fd, period_fd, duty_cycle_fd, enable_fd;
	duty_cycle_fd = open("/sys/class/pwm/pwmchip4/pwm0/duty_cycle", O_WRONLY);
    write(duty_cycle_fd, "500000000", 9);
    close(duty_cycle_fd);
	sleep(1);
    // Ativar o canal PWM
    enable_fd = open("/sys/class/pwm/pwmchip4/pwm0/enable", O_WRONLY);
    write(enable_fd, "1", 1);
    close(enable_fd);
}
void led_blue_of(){//p9_21
	int export_fd, period_fd, duty_cycle_fd, enable_fd;
...
int main() {
	...
	while(1){
		...
		if(celsius < 25.00){
			...
			led_red_of();
			led_blue_on();//p9_22
		}else if(celsius >= 25.00 && celsius < 30){
			led_blue_of();
			led_red_of();
			led_green_on();//p8_19
		}else{
